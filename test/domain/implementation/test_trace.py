...
def test_end_corrected(trace: CorrectableTrace, corrected: CorrectableTrace) -> None:
...
def test_split_with_color_turn(target: SplitableTrace, colored: ColoredTrace) -> None:
...
async def test_to_move(fen: str, movement: IMovement, next_fen: str) -> None:
...
async def test_produced(fens: list[str], status: IStatus, colored: ColoredTrace, producable: ITraceProducable) -> None:
    assert (
        await ProducableTrace(status, FakeWhiteMovement(), FakeBlackMovement(), producable).produced_with_spliting(fens)
        == colored
    )
