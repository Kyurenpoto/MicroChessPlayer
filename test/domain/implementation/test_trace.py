...
def test_index() -> None:
    assert Trace([[STARTING_FEN], [FIRST_FEN]], [[FIRST_SAN], []], [[0], [0]]).indexed([0]) == Trace(
...
def test_color(color: str, fen: str, sanlist: List[str]) -> None:
    assert Trace([[STARTING_FEN], [FIRST_FEN]], [[FIRST_SAN], []], [[0], [0]]).colored(color) == Trace(
        [[fen]], [sanlist], [[0]]
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
            Trace([[STARTING_FEN, FIRST_FEN, STARTING_FEN]], [[FIRST_SAN, FIRST_SAN]], [[0, 0]]),
            Trace([[STARTING_FEN, STARTING_FEN]], [[FIRST_SAN]], [[0]]),
            Trace([[FIRST_FEN]], [[FIRST_SAN]], [[0]]),
        ),
        (
            Trace([[STARTING_FEN, FIRST_FEN]], [[FIRST_SAN]], [[0]]),
...
def test_split_with_color_turn(target: Trace, white: Trace, black: Trace) -> None:
